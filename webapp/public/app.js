...
async function runDecorGenerate() {
  ...
  try {
    const product =
      state.productType === 'poster' ? 'poster' : state.productType === 'tapisserie' ? 'tapestry' : 'canvas'
    lastDecor = await callDecorJob({ image: state.imageDataUrl, target: state.orientation, product })
