...
function updateDropzoneLock() {
  ...
  const b = $('#dzInner b')
  if (b) b.textContent = locked ? "Choisissez d'abord le produit à refaire" : 'Touchez pour choisir une image'
}
...
function renderProductImages(ctx) {
  ...
  row.innerHTML = imgs
    .map((im) => `<img class="current-thumb" src="${escapeHtml(im.url)}" alt="${escapeHtml(im.alt || '')}" loading="lazy">`)
    .join('')
...
function onDragMove(e) {
  ...
  if (!drag.armed) {
    ...
    if (Math.abs(e.clientX - drag.startX) + Math.abs(e.clientY - drag.startY) > MOVE_SLOP) cancelDrag()
    return
  }
...
function attachDrag(cell, res) {
  cell.addEventListener('pointerdown', (e) => {
    ...
    drag = {
      id: cell.dataset.id, cell, res, pointerId: e.pointerId,
    ...
    drag.timer = setTimeout(() => { if (drag) armDrag() }, ARM_MS)
