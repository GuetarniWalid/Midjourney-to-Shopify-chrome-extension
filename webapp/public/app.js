...
const fileInput = $('#fileInput'), dropzone = $('#dropzone'), dzInner = $('#dzInner'),
      sourcePreview = $('#sourcePreview'), changeBtn = $('#changeImageBtn');
...
function detectOrientation(w, h) {
  const ratio = w / h;
  let ori, label;
  if (ratio >= 0.95 && ratio <= 1.05) { ori = 'square'; label = 'Carré'; }
  else if (ratio < 0.95) { ori = 'portrait'; label = 'Portrait'; }
  else { ori = 'landscape'; label = 'Paysage'; }
  state.orientation = ori;
  const badge = $('#orientationBadge');
  badge.textContent = label; badge.className = 'badge ' + ori; badge.classList.remove('hidden');
}

/* ---------- 2. Type produit + collections ---------- */
$$('#productType .seg-btn').forEach(btn => btn.addEventListener('click', () => {
  ...
  if (state.results.length && !confirm('Changer de type de produit va effacer les rendus déjà générés. Continuer ?')) return;
...
function clearResults() {
  for (const r of state.results) fetch(RENDER + '/api/upload/' + r.url.split('/').pop(), { method: 'DELETE' }).catch(() => {});
  state.results = [];
  renderResults(); refreshAction();
}

const collSearch = $('#collectionSearch'), collList = $('#collectionList'), collChosen = $('#collectionChosen');
...
async function loadCollections() {
  ...
  try {
    ...
    state.collections = (data && data.success && data.data) ? data.data : (Array.isArray(data) ? data : []);
  } catch { state.collections = []; }
  if (!state.collections.length) collSearch.placeholder = 'Collections indisponibles (backend hors-ligne)';
}

function clearCollection() {
  state.collection = null; collChosen.classList.add('hidden'); collChosen.innerHTML = '';
  collSearch.value = ''; collSearch.classList.remove('hidden'); refreshAction();
}

collSearch.addEventListener('input', () => {
  const term = collSearch.value.toLowerCase().trim();
  const list = term ? state.collections.filter(c => (c.title || '').toLowerCase().includes(term)) : state.collections;
  collList.innerHTML = list.slice(0, 50).map(c => `<div class="combo-item" data-id="${c.id}">${escapeHtml(c.title)}</div>`).join('')
    || `<div class="combo-item" style="color:var(--muted)">Aucune collection</div>`;
...
collList.addEventListener('click', e => {
  const item = e.target.closest('.combo-item'); if (!item || !item.dataset.id) return;
  const col = state.collections.find(c => String(c.id) === item.dataset.id);
  if (!col) return;
  state.collection = col;
  collList.classList.add('hidden'); collSearch.classList.add('hidden');
  collChosen.innerHTML = `<span>${escapeHtml(col.title)}</span><span class="x">✕</span>`;
  collChosen.classList.remove('hidden');
  refreshAction();
});
collChosen.addEventListener('click', e => { if (e.target.classList.contains('x')) clearCollection(); });
document.addEventListener('click', e => { if (!e.target.closest('.combo')) collList.classList.add('hidden'); });
...
function renderMockups() {
  const grid = $('#mockupGrid'), hint = $('#mockupsHint');
  grid.innerHTML = '';
  if (!state.orientation) { hint.textContent = 'Choisissez une image d\'abord'; return; }
  ...
  for (const cat of state.templates) {
    ...
    const head = document.createElement('div'); head.className = 'mockup-cat'; head.textContent = cat.name;
  ...
  hint.textContent = count ? `${count} mockup(s) en ${labelOri(ori)}` : 'Aucun mockup pour cette orientation';
...
async function generate(catName, sub, layout, cell) {
  ...
  const spin = document.createElement('div'); spin.className = 'spin'; spin.innerHTML = '<div class="loader"></div>';
  cell.appendChild(spin);
  try {
    const r = await fetch(RENDER + '/api/render', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ psd: layout.psd, image: state.imageDataUrl, mockupContext: sub.context || `${catName} - ${sub.name}` }),
    });
    const data = await r.json();
    if (!data.success) throw new Error(data.error || 'échec du rendu');
    state.results.push({ id: 'r' + Date.now() + Math.random().toString(36).slice(2, 5), path: data.url, url: renderUrl(data.url), context: data.mockupContext, label: sub.name });
    renderResults(); refreshAction();
    toast('Rendu ajouté ✓', 'ok');
  } catch (e) {
    toast('Erreur : ' + e.message, 'err');
  } finally {
    cell.classList.remove('busy'); spin.remove();
  }
}

/* ---------- 4. Résultats + drag-drop UNIFIÉ (pointer events : souris === tactile) ---------- */
function renderResults() {
  const grid = $('#resultsGrid'), empty = $('#resultsEmpty');
  grid.innerHTML = '';
  empty.classList.toggle('hidden', state.results.length > 0);
  state.results.forEach((res, i) => {
    const cell = document.createElement('div');
    cell.className = 'result-cell'; cell.dataset.id = res.id;
    cell.innerHTML = `<div class="num">${i + 1}</div><button class="del" title="Supprimer">✕</button><img src="${res.url}" alt="" draggable="false">`;
    cell.querySelector('.del').addEventListener('click', ev => { ev.stopPropagation(); removeResult(res.id); });
    attachDrag(cell, res);
    grid.appendChild(cell);
  });
}
